  ...
  $.fn.overflowing = function (options, callback) {
    ...
    this.each(function () {
      ...
      for (var i = 0; i < parents.length; i++) {
        ...
        var absPosition = !!~["absolute", "fixed"].indexOf(
          $parent.css("position")
        );
        var parentPosition = $parent.position();
        var parentWidth = $parent.width();
        var parentHeight = $parent.height();
        var parentToBottom = absPosition
          ? parentHeight
          : parentHeight + parentPosition.top;
        var parentToRight = absPosition
          ? parentWidth
          : parentWidth + parentPosition.left;
